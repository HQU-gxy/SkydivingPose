...
def visualize_and_save_keypoints_with_connections(frame, keypoints, name, output_dir, frame_index):  
...
def get_pose2D(video_path, detect_path, output_dir):
    import json
    print(video_path)
    all_kpts = np.zeros((1, 165, 17, 2), dtype=np.float32) 
    all_socre = np.ones((1, 165, 17), dtype=np.float32) 
    frame_index = 0
    for filename in os.listdir(video_path):  
        if filename.endswith('.png'):  
            ...
            for index, kp in  enumerate(data["shapes"]):
                print(index, kp['points'][0])
                kpts[index] = kp['points'][0]
            # print(kpts)
            frame = cv2.imread(image_path)
            for ii in range(3):
                all_kpts[0][frame_index] = kpts
                frame_index += 1
                visualize_and_save_keypoints_with_connections(frame, kpts, name, output_dir / "images", frame_index)

    print(all_kpts.shape)

    keypoints, scores, valid_frames = h36m_coco_format(all_kpts, all_socre)
    re_kpts = revise_kpts(keypoints, scores, valid_frames)
    print('Generating 2D pose successfully!')

    # output_dir += 'input_2D/'
    os.makedirs(output_dir / 'input_2D/', exist_ok=True)

    output_npz = output_dir / 'input_2D/input_keypoints_2d.npz'
...
def get_pose3D(video_path, output_dir, fix_z):
    ...
    for i in tqdm(range(n_chunks)):
        ...
        pad_right = max(0, end_index - video_length)  

        # 获取要处理的关键点数据 
        print("=====> keypoints", keypoints.shape) 
        input_2D_no = keypoints[0][low_index:high_index]  

        # 使用 'edge' 模式进行填充，以保证具有 args.frames 长度  
        if pad_left > 0 or pad_right > 0:  
            input_2D_no = np.pad(input_2D_no, ((pad_left, pad_right), (0, 0), (0, 0)), 'edge')  

        # 修剪或进一步补齐数据以确保长度正好是 args.frames  
        if input_2D_no.shape[0] > args.frames:  
            input_2D_no = input_2D_no[:args.frames]  
        elif input_2D_no.shape[0] < args.frames:  
            additional_pad = args.frames - input_2D_no.shape[0]  
            input_2D_no = np.pad(input_2D_no, ((0, additional_pad), (0, 0), (0, 0)), 'edge')
        ...
        # print("low_index, high_index, pad_left, pad_right", start_index, end_index, low_index, high_index, pad_left, pad_right)
        ...
        print(input_2D[0][1][0])
        print(input_2D[0][1][-1])
...
def img2video(video_path, output_dir, video_name="output"):
    cap = cv2.VideoCapture(video_path)
...
if __name__ == "__main__":
    ...
    video_path = file_path / "pose_image/"
    pose_path = file_path / "pose/"
    output_dir = file_path / "pose_output/"
    # get_pose2D(video_path, pose_path, output_dir)
    output_dir = "/home/zlt/Documents/SkydivingPose/sample/labelme/pose_output/"
    video_path = Path("/home/zlt/Documents/SkydivingPose/sample/labelme/pose_output/input.mp4")
    print("=============")
    get_pose3D(str(video_path), str(output_dir), args.fix_z)
    img2video(str(video_path), str(output_dir))